...
- menjaga tautan antarmateri tetap valid;
- meningkatkan kualitas secara bertahap tanpa menghapus pengetahuan yang masih bernilai.

## Batas Repository

`public` hanya memuat materi yang dimaksudkan untuk konsumsi publik. Konten pribadi, catatan personal, eksperimen privat, dan repository pembelajaran pribadi berada di repository privat `independentlearners/file` di bawah `Pribadi/saya/`.

Karena `public` sering dikloning secara langsung oleh orang lain, struktur dan tautan di dalamnya harus dapat dipahami tanpa bergantung pada workspace privat.
...
Materi dapat berasal dari dokumentasi resmi, eksperimen, kursus, komunitas, atau bantuan AI. Bantuan AI tidak dengan sendirinya menjadikan sebuah pernyataan terverifikasi.
...
3. periksa referensi antardomain;
4. pindahkan atau perbarui target terlebih dahulu jika diperlukan;
5. lakukan audit tautan setelah perubahan.

`public` tidak boleh mempunyai tautan yang mengharuskan pembaca mengakses `independentlearners/file` untuk memahami jalur publik.
